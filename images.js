...
$(".images img").mouseenter(this, function(event) {
  if(!block && currentEvent == "enter") {
    block = true;
    let t = this;
  
    $(t).addClass("gray current");
  
    let image = getImageById(t.id);
    let preview = $(`
      <div id="preview">
        <img src="images/medium/${image.path}">
        <p>${image.title} <br> ${image.city}, ${image.country} ${image.taken}</p>
      </div>`);   
    ...
    preview.css( "left", x + offset);
    preview.css( "top", y + offset);
...
$(".images img").mouseleave(this, function(event) {

  if(!block && currentEvent == "leave") {
    block = true;

    let t = this; 
    $(t).removeClass("gray current");
...
$(".images img").mousemove(this, function(event) {
  let x = event.pageX;
  let y = event.pageY;
